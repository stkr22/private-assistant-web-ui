...
interface EditSyncJobProps {
  job: ImmichSyncJobPublic
}

const EditSyncJob = ({ job }: EditSyncJobProps) => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      strategy: job.strategy ?? "RANDOM",
      query: job.query ?? "",
      count: job.count ?? 10,
      random_pick: job.random_pick ?? false,
      overfetch_multiplier: job.overfetch_multiplier ?? 3,
      min_color_score: job.min_color_score ?? 0.5,
      is_active: job.is_active ?? true,
      is_favorite: job.is_favorite ?? undefined,
      city: job.city ?? "",
      state: job.state ?? "",
      country: job.country ?? "",
      taken_after: formatDateForInput(job.taken_after),
      taken_before: formatDateForInput(job.taken_before),
  ...
  const mutation = useMutation({
    mutationFn: (data: ImmichSyncJobUpdate) =>
      ImmichSyncJobsService.updateSyncJob({
        jobId: job.id,
        requestBody: data,
      }),
    onSuccess: () => {
      showSuccessToast("Sync job updated successfully")
      setIsOpen(false)
    },
  ...
  const onSubmit = (data: FormData) => {
    const updateData: ImmichSyncJobUpdate = {
      name: data.name,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Sync Job</DialogTitle>
          <DialogDescription>
            Update the sync job configuration.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
