...
interface EditIntentPatternProps {
  pattern: app__models_commons_api__IntentPatternPublic
  open: boolean
  onOpenChange: (open: boolean) => void
}

function EditIntentPattern({
  pattern,
  open,
  onOpenChange,
}: EditIntentPatternProps) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      keywords: pattern.keywords.map((kw) => ({
        keyword: kw.keyword,
        keyword_type: (kw.keyword_type as "primary" | "negative") || "primary",
        is_regex: kw.is_regex ?? false,
  ...
  const mutation = useMutation({
    mutationFn: (data: IntentPatternUpdate) =>
      IntentPatternsService.updateIntentPattern({
        patternId: pattern.id,
        requestBody: data,
      }),
    onSuccess: () => {
      showSuccessToast("Intent pattern updated successfully")
      onOpenChange(false)
    },
  ...
  const onSubmit = (data: FormData) => {
    mutation.mutate(data as IntentPatternUpdate)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Intent Pattern</DialogTitle>
          <DialogDescription>
            Update the voice command intent pattern configuration.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
...
export default EditIntentPattern
